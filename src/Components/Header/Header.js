    ...
    return (
        <section className=''>
            <nav className='bg-gradient-to-r from-indigo-800 via-purple-600 to-blue-300 py-2 px-4 md:px-8 relative'>
                <div onClick={() => setOpen(!open)} className='text-blue-600 w-10 absolute mt-1 right-4 md:right-14 md:hidden'>
                    {
                ...
                <div className='flex md:items-center md:justify-end flex-col md:flex-row items-start font-bold'>
                    <div className='flex justify-between items-center flex-grow'>
                        <NavLink to='/' className='flex-grow flex items-center font-bold text-2xl text-blue-600'> <img src={logo} className='w-44' alt="" /></NavLink>

                        <Link className='inline ml-16 md:hidden' onClick={() => setDarkMod(!darkMod)}>
                            {
                    ...
                    <div className={`flex md:items-center md:justify-end flex-col md:flex-row items-start text-black font-bold ml-10 mt-3 md:m-0 md:block duration-500 ease-in ${open ? 'block' : 'hidden'}`}>
                        ...
                        <Link className='md:inline mr-10 hidden md:block' onClick={() => setDarkMod(!darkMod)}>
                            {
                        ...
                        <NavLink onClick={() => setOpen(!open)} to='/home'
                            className={({ isActive }) => isActive ?
                                'underline underline-offset-4  decoration-2 text-rose-700 md:mr-10' : 'md:mr-10 hover:text-blue-700'}>Home</NavLink>
                        <NavLink onClick={() => setOpen(!open)} to='/courses'
                            className={({ isActive }) => isActive ?
                                'mt-3 underline underline-offset-4 decoration-2 text-rose-700 md:mr-10' : 'md:mr-10 mt-3 hover:text-blue-700'}>Courses</NavLink>
                        <NavLink onClick={() => setOpen(!open)} to='/faq'
                            className={({ isActive }) => isActive ?
                                'mt-3 underline underline-offset-4 decoration-2 text-rose-700 md:mr-10' : 'md:mr-10 mt-3 hover:text-blue-700'}>FAQ</NavLink>
                        <NavLink onClick={() => setOpen(!open)} to='/blog'
                            className={({ isActive }) => isActive ?
                                'mt-3 underline underline-offset-4 decoration-2 text-rose-700 md:mr-10' : 'md:mr-10 mt-3 hover:text-blue-700'}>Blog</NavLink>
                        ...
                        {
                            user?.uid && <NavLink onClick={() => setOpen(!open)} className='md:hidden mt-3'>
                                <button onClick={() => handleLogOut()} type="button" class="inline-block px-4 py-1.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">Log out</button>
                            </NavLink>
                        }
            ...
            {
                user?.uid && <div className={`absolute z-50 top-14 right-8 w-56 ${isHovering || profileView ? 'block' : 'hidden'}`}>
